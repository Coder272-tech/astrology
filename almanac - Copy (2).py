...
#   345° 驚蟄
#
# The term is determined from the Sun's APPARENT GEOCENTRIC
# ECLIPTIC LONGITUDE, not from the calendar date.
# ---------------------------------------------------------------------------

SOLAR_TERMS = [
    (0.0,   "春分", "Chun Fen",  "Spring Equinox"),
    (15.0,  "清明", "Qing Ming", "Clear and Bright"),
    (30.0,  "穀雨", "Gu Yu",     "Grain Rain"),
    (45.0,  "立夏", "Li Xia",    "Start of Summer"),
    (60.0,  "小滿", "Xiao Man",  "Grain Full"),
    (75.0,  "芒種", "Mang Zhong","Grain in Ear"),
    (90.0,  "夏至", "Xia Zhi",   "Summer Solstice"),
    (105.0, "小暑", "Xiao Shu",  "Minor Heat"),
    (120.0, "大暑", "Da Shu",    "Major Heat"),
    (135.0, "立秋", "Li Qiu",    "Start of Autumn"),
    (150.0, "處暑", "Chu Shu",   "End of Heat"),
    (165.0, "白露", "Bai Lu",    "White Dew"),
    (180.0, "秋分", "Qiu Fen",  "Autumn Equinox"),
    (195.0, "寒露", "Han Lu",   "Cold Dew"),
    (210.0, "霜降", "Shuang Jiang","Frost Descent"),
    (225.0, "立冬", "Li Dong",  "Start of Winter"),
    (240.0, "小雪", "Xiao Xue", "Minor Snow"),
    (255.0, "大雪", "Da Xue",   "Major Snow"),
    (270.0, "冬至", "Dong Zhi",  "Winter Solstice"),
    (285.0, "小寒", "Xiao Han", "Minor Cold"),
    (300.0, "大寒", "Da Han",   "Major Cold"),
    (315.0, "立春", "Li Chun",   "Start of Spring"),
    (330.0, "雨水", "Yu Shui",   "Rain Water"),
    (345.0, "驚蟄", "Jing Zhe", "Awakening of Insects"),
]
...
def get_sun_ecliptic_longitude(t):
    """
...
def get_solar_term(t):
    """
    Determine the current 15° solar-term sector from the Sun's
    ...
    Example:
        ...
        current term = 白露
        term boundary = 165°
        degrees since term = 2°

    Returns a dictionary containing the complete solar-term state.
    """

    sun_lon, sun_lat, sun_distance = get_sun_ecliptic_longitude(t)

    sun_lon %= 360.0

    # Every solar term is exactly 15° apart in solar longitude.
    index = int(sun_lon // 15.0)

    term_longitude, chinese, pinyin, english = SOLAR_TERMS[index]
    ...
    (
        ...
        next_pinyin,
        next_english,
    ) = SOLAR_TERMS[next_index]

    degrees_since = (sun_lon - term_longitude) % 360.0
    degrees_until_next = (next_longitude - sun_lon) % 360.0
    ...
    return {
        ...
        "longitude": term_longitude,

        "sun_longitude": sun_lon,
        "sun_latitude": sun_lat,
        "sun_distance_au": sun_distance,

        "degrees_since": degrees_since,
        "degrees_until_next": degrees_until_next,

        "next_name": next_chinese,
...
def print_solar_term(t):

    solar = get_solar_term(t)
    ...
    print(
        f"Term longitude    : "
        f"{solar['longitude']:.8f}°"
    )

    print(
        f"Sun longitude     : "
        f"{solar['sun_longitude']:.8f}°"
    )

    print(
        f"Sun latitude      : "
        f"{solar['sun_latitude']:.8f}°"
    )

    print(
        f"Degrees since term: "
        f"{solar['degrees_since']:.8f}°"
    )

    print(
        f"Degrees to next   : "
        f"{solar['degrees_until_next']:.8f}°"
    )
    ...
    print(
        ...
        f"{solar['next_name']} "
        f"{solar['next_pinyin']} "
        f"({solar['next_english']})"
    )

    print(
        f"Next longitude    : "
        f"{solar['next_longitude']:.8f}°"
    )
